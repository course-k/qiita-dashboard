...
router.get('/summary', (_req, res) => {
  ...
  const row = db.prepare(`
    SELECT
      ...
      SUM(COALESCE(page_views_count, 0)) as total_pv
    FROM articles
...
router.get('/trend', (req, res) => {
  ...
  let rows: Array<{ month: string; count: number }>

  if (metric === 'pv') {
    // PVはスナップショットから月次増分を計算
    ...
    rows = db.prepare(`
      ...
      FROM (
        SELECT
          article_id,
          snapshot_date,
          pv_count - LAG(pv_count, 1, 0) OVER (PARTITION BY article_id ORDER BY snapshot_date) as pv_count
        FROM pv_snapshots
      )
      WHERE snapshot_date BETWEEN ? AND ? AND pv_count > 0
      GROUP BY month
      ORDER BY month
    `).all(fromDate, toDate) as Array<{ month: string; count: number }>
  } else {
    const table = metric === 'stocks' ? 'stock_events' : 'like_events'
    const fromDate = from ? `${from}-01` : '2000-01-01'
    const toDate = to ? `${to}-31` : '2099-12-31'
    rows = db.prepare(`
      SELECT
